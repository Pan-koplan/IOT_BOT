#pragma message("#include \"imu_sensor_broadcaster_parameters.hpp\" is deprecated. Use #include <imu_sensor_broadcaster/imu_sensor_broadcaster_parameters.hpp> instead.")
...
[[nodiscard]] auto to_parameter_value(rsl::StaticVector<T, capacity> const& value) {
    ...
    struct Params {
        ...
        bool use_namespace_as_sensor_name_prefix = false;
  ...
  public:
    ...
    StackParams get_stack_params() {
      ...
      StackParams output;
      output.use_namespace_as_sensor_name_prefix = params.use_namespace_as_sensor_name_prefix;
    ...
    rcl_interfaces::msg::SetParametersResult update(const std::vector<rclcpp::Parameter> &parameters) {
      ...
      for (const auto &param: parameters) {
        ...
        if (param.get_name() == (prefix_ + "use_namespace_as_sensor_name_prefix")) {
            updated_params.use_namespace_as_sensor_name_prefix = param.as_bool();
            RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
        }
      }
    ...
    void declare_params(){
      ...
      if (!parameters_interface_->has_parameter(prefix_ + "tf_frame_prefix")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "(optional) Prefix to be appended to the tf frames, will be added to the sensor's frame_id before publishing. If the parameter is empty, controller's namespace will be used.";
          descriptor.read_only = false;
      ...
      if (!parameters_interface_->has_parameter(prefix_ + "use_namespace_as_sensor_name_prefix")) {
          rcl_interfaces::msg::ParameterDescriptor descriptor;
          descriptor.description = "If true the '/namespace/' is added to the sensor name which causes changes in interfaces names e. g. /namespace/sensor_name/orientation.x";
          descriptor.read_only = false;
          auto parameter = to_parameter_value(updated_params.use_namespace_as_sensor_name_prefix);
          parameters_interface_->declare_parameter(prefix_ + "use_namespace_as_sensor_name_prefix", parameter, descriptor);
      }
      ...
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      updated_params.tf_frame_prefix = param.as_string();
      param = parameters_interface_->get_parameter(prefix_ + "use_namespace_as_sensor_name_prefix");
      RCLCPP_DEBUG_STREAM(logger_, param.get_name() << ": " << param.get_type_name() << " = " << param.value_to_string());
      updated_params.use_namespace_as_sensor_name_prefix = param.as_bool();
